...
ALTER PROCEDURE [dbo].[sp_LoadProgressCompletion] AS
BEGIN
	--0. ensure temp table doesn't exist in use
	IF OBJECT_ID(N'tempdb..#st_math_pc') IS NOT NULL
	BEGIN
		DROP TABLE #st_math_pc
	END
    ...
    --1. bulk load csv and SELECT INTO temp table
        ...
        FROM
            ...
            FROM OPENROWSET(
						...
						'text;Database=C:\robots\ST_Math\data\from_ST_Math\for_Silo', 
						'SELECT * FROM progress.csv'
						)
            ) sub;
        ...
        MERGE progress_completion target
        USING new_file staging
                            ON  (target.SCD = staging.SCD)
							AND (target.week_ending_date = staging.week_ending_date)
							AND (target.GCD = staging.GCD)
							AND (target.UUID = staging.UUID)
        WHEN MATCHED THEN
                    UPDATE SET
							...
							target.week_ending_date			= staging.week_ending_date
